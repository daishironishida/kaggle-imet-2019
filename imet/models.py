...
class DenseNet(nn.Module):
    def __init__(self, num_classes,
                 pretrained=False, net_cls=M.densenet121, dropout=0):
        super().__init__()
        self.net = create_net(net_cls, pretrained=pretrained)
...
class SEResNeXt(nn.Module):
    def __init__(self, num_classes,
                 pretrained=False, net_cls="seresnext101", dropout=0):
        super(SEResNeXt, self).__init__()

        if net_cls == "seresnext101":
            layers = [3, 4, 23, 3]

        self.base = SENet(SEResNeXtBottleneck, layers, groups=32, reduction=16,
              dropout_p=None, inplanes=64, input_3x3=False,
              downsample_kernel_size=1, downsample_padding=0,
              num_classes=1000)

        if ON_KAGGLE and pretrained:
            self.base.load_state_dict(torch.load('../input/pytorch-model-zoo/se_resnext101_32x4d-3b2fe3d8.pth'))
        self.base.last_linear = nn.Linear(2048, num_classes)
...
seresnext101 = partial(SEResNeXt, net_cls="seresnext101")